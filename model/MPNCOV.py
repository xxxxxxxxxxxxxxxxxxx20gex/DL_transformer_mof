...
class Covpool(Function):
    ...
    def forward(ctx, input):
        ...
        I_hat = I_hat.view(1, M, M).repeat(batchSize, 1, 1).type(x.dtype) # (b,m,m)
        y = x.bmm(I_hat).bmm(x.transpose(1, 2))   # （batchsize, dim, dim）
...
class Sqrtm(Function):
    ...
    def forward(ctx, input, iterN):
        x = input    # （batchsize, dim, dim）
        batchSize = x.data.shape[0]
        dim = x.data.shape[1]
        dtype = x.dtype
        I3 = 3.0 * torch.eye(dim, dim, device=x.device).view(1, dim, dim).repeat(batchSize, 1, 1).type(dtype)  #（batchsize, dim, dim）
...
class Triuvec(Function):
    ...
    def forward(ctx, input):
        ...
        # 使用 torch.triu_indices 替代手动构造 mask
        indices = torch.triu_indices(dim, dim, device=x.device)

        # 重塑并直接使用向量化操作
        x = x.reshape(batchSize, dim * dim)
        y = x[:, indices[0] * dim + indices[1]]
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros(batchSize, dim, dim, device=x.device, requires_grad=False)
        grad_input = grad_input.reshape(batchSize, dim * dim)
        grad_input[:, indices[0] * dim + indices[1]] = grad_output
        grad_input = grad_input.reshape(batchSize, dim, dim)
