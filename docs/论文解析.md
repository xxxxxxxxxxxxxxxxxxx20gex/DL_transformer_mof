...
# 微调
- 第一列：MOFid 描述串，包含有机构件/金属节点的 SMILES 片段与拓扑标识，格式大致为“若干 SMILES 片段用点号拼接 + '&&' + 拓扑名与类别”，例如 ...&&pcu.cat0（pcu 为拓扑，cat0/cat1/cat2 为类别标签/变体）。
- 第二列：该结构对应的目标数值。就本文件名 hMOF_CH4_0.5 来看，这一列表示甲烷在 0.5 bar 条件下的性质指标（通常是吸附量/容量，常见单位 mmol/g；具体单位以原始数据说明为准）。